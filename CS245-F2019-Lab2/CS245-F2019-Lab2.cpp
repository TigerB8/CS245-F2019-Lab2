...
int main()
{
	...
	string temp[] = {"Holmes & Watson", "Mega Shark vs. Crocosaurus", "C-SPAN: The Movie", "That's My Boy"};
	vector<string> movies(begin(temp), end(temp));

	// Add a 5th string to the end of the vector
	movies.push_back("Titanic 2: The Quickening");

	// Announce the nominees
	cout << "And the nominees for Best Picture (cough) are..." << std::endl;

	// Loop through the vector; print each vector element to console.
	for (int i = 0; i < movies.size; i++)
	{
		cout << movies.at(i) << std::endl;
	}
	...
	for (char c : winner)
	{
		winner = putchar(toupper(c));
	}

	// Print the UPPERCASE string
	cout << std::endl << "And the winner is..." << winner;
